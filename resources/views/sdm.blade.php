...
@section('content')
    ...
    <article class="container mt-28">
        ...
        @if ($lectures->isEmpty())
            <div class="text-center text-xneutral-300 font-montserrat text-medium text-sm sm:text-base mt-10">
                No Data Available
            </div>
        @else
        ...
        @else
            <div class="mt-10 space-y-11">
                @foreach ($admins as $admin)
                    <div class="flex flex-col items-center sm:flex-row gap-8 sm:gap-16">
                        <img class="sm:col-span-2" src="{{ 'storage/' . $admin->image }}" alt="{{ $admin->nama }}" />
                        <div class="sm:col-span-10 font-montserrat">
                            <h3 class="text-base sm:text-lg font-semibold text-primary-200">
                                {{ $admin->nama }}
                            </h3>
